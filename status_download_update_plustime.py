		...
		try:
			for each in content:
				...
				if transfer_author_pre and image_url:
					...
					insstr="insert into userStatusPlusTime(uuid,tran_content,tran_author,image_url,tran_reason,time) values(%s,'%s','%s','%s','%s','%s'); "%(id,transfer_content,transfer_author,image_url[0],transfer_reason,time)
					...
					time=formatTime(time.encode('utf-8'))
					insstr="insert into userStatusPlusTime(uuid,tran_content,tran_author,tran_reason,time) values(%s,'%s','%s','%s','%s'); "%(id,transfer_content,transfer_author,transfer_reason,time)
				elif (not transfer_author_pre) and image_url:
					content1= each.xpath('div[1]/span[@class="ctt"]/text()')[0].encode('utf-8')
					time=each.xpath('div[2]/span[@class="ct"]/text()')[0]
					time=formatTime(time.encode('utf-8'))
					insstr="insert into userStatusPlusTime(uuid,content,image_url,time) values(%s,'%s','%s','%s'); "%(id,content1,image_url[0],time)
				elif (not transfer_author_pre) and (not image_url):
					content1= each.xpath('div[1]/span[@class="ctt"]/text()')[0].encode('utf-8')
					time=each.xpath('div[1]/span[@class="ct"]/text()')[0]
					time=formatTime(time.encode('utf-8'))
					insstr="insert into userStatusPlusTime(uuid,content,time) values(%s,'%s','%s'); "%(id,content1,time)
